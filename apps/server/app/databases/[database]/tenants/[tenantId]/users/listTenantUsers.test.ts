...
describe("list tenant users", () => {
  it("404s if the user is not in the tenant", async () => {
    ...
    expect(runCommands).toEqual([
      "SELECT COUNT(*) FROM users.tenant_users WHERE user_id = some-uuid AND tenant_id = tenantId AND deleted IS NULL",
    ]);
  });
  it("returns a list of tenant users", async () => {
    ...
    expect(runCommands).toEqual([
      "SELECT COUNT(*) FROM users.tenant_users WHERE user_id = some-uuid AND tenant_id = tenantId AND deleted IS NULL",
      'SELECT id, u.email, name, family_name AS "familyName", given_name AS "givenName", picture FROM users.users u JOIN users.tenant_users tu ON u.id = tu.user_id WHERE tu.tenant_id = tenantId AND u.deleted IS NULL AND tu.deleted IS NULL',
    ]);
