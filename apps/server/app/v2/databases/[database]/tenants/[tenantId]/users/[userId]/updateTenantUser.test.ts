...
describe("update tenant user", () => {
  it("404s if the user is not in the tenant", async () => {
    ...
    expect(runCommands).toEqual([
      ":SET LOCAL nile.tenant_id = '019073f4-75a6-72b9-a379-5ed38ca0d01a'; :SET LOCAL nile.user_id = 'some-uuid'; SELECT COUNT(*) FROM users.tenant_users WHERE deleted IS NULL AND user_id = some-uuid",
    ]);
  });
  ...
  it("returns an updated user", async () => {
    ...
    expect(runCommands).toEqual([
      ":SET LOCAL nile.tenant_id = '019073f4-75a6-72b9-a379-5ed38ca0d01a'; :SET LOCAL nile.user_id = 'some-uuid'; SELECT COUNT(*) FROM users.tenant_users WHERE deleted IS NULL AND user_id = some-uuid",
      ":SET LOCAL nile.tenant_id = '019073f4-75a6-72b9-a379-5ed38ca0d01a'; :SET LOCAL nile.user_id = '0190b7cd-661a-76d4-ba6e-6ae2c383e3c1'; SELECT COUNT(*) FROM users.tenant_users WHERE deleted IS NULL AND user_id = some-uuid",
