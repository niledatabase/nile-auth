...
CREATE TABLE auth.invites (
  ...
  token TEXT NOT NULL,
  identifier TEXT NOT NULL, -- required: whom the invite is for
  roles TEXT,
  created_by UUID,
  expires TIMESTAMP WITH TIME ZONE,
  CONSTRAINT auth_invites_pkey PRIMARY KEY("tenant_id","id")
);

CREATE UNIQUE INDEX unique_invite_per_email_per_tenant
  ON auth.invites (tenant_id, identifier);

INSERT INTO auth.invites (
  tenant_id,
  code,
  email,
  roles,
...
export async function PUT(
  ...
  try {
    console.log("we in her esome where");
    const [session] = await auth(req);
    if (session && session?.user?.id) {
      const { tenantId } = params;
      if (!tenantId) {
